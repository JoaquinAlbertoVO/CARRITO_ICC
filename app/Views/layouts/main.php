                                    ...
                                    <div class="main-menu__right">
                                        ...
                                        <div class="main-menu__right-cart-search">
                                            ...
                                            <div class="main-menu__right-search-box" style="margin-right: 15px; display: flex; align-items: center; position: relative;">
                                                <a href="#" class="btnOpenUbigeo" aria-label="Ubicación" style="color: white; display: flex; align-items: center; gap: 8px; text-decoration: none; margin-right: 15px;" title="Cambiar mi ubicación">
                                                    ...
                                                    <div style="display: flex; flex-direction: column; font-size: 13px; line-height: 1.2; text-align: left;">
                                                        ...
                                                        <span class="headerLocationText" style="font-weight: bold; color: #4a90e2;">Lima, Lima</span>
                                                    </div>
                                                </a>
                                                <!-- Popover de Ubicación -->
                                                <div class="locationPopover" style="display: none; position: absolute; top: calc(100% + 10px); left: 0; background: white; border-radius: 12px; padding: 20px; box-shadow: 0 10px 25px rgba(0,0,0,0.2); z-index: 999999; width: 280px; text-align: left; cursor: default;">
                                                    <div style="font-weight: 700; color: #172A4E; font-size: 15px; margin-bottom: 8px;">Estás navegando en:</div>
                                                    <div class="popoverLocationText" style="color: #666; font-size: 14px; margin-bottom: 20px; line-height: 1.4;">Lima, Lima</div>
                                                    <div style="display: flex; gap: 10px;">
                                                        <button class="btnConfirmLocation" style="flex: 1; background: #172A4E; color: white; border: none; border-radius: 20px; padding: 8px 10px; font-size: 13px; font-weight: bold; transition: background 0.3s;">Confirmar</button>
                                                        <button class="btnChangeLocation" style="flex: 1; background: transparent; color: #172A4E; border: 1px solid #172A4E; border-radius: 20px; padding: 8px 10px; font-size: 13px; font-weight: bold; transition: background 0.3s;">Cambiar</button>
                                                    </div>
                                                    <!-- Flecha -->
                                                    ...
                                                    <!-- Cerrar -->
                                                    <button class="btnClosePopover" style="position: absolute; top: 10px; right: 15px; background: none; border: none; font-size: 18px; color: #999; line-height: 1;">&times;</button>
                                                </div>
                                            </div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.addEventListener("DOMContentLoaded", function() {
            ...
            .then(data => {
                ...
                document.getElementById('ubigeoForm').addEventListener('submit', function(e) {
                    ...
                    fetch('<?= BASE_URL ?>visitor/saveLocation', {
                        ...
                        document.querySelectorAll('.headerLocationText').forEach(el => el.innerText = locText);
                        myModal.hide();
        ...
        if (savedLocText) {
            document.querySelectorAll('.headerLocationText').forEach(el => el.innerText = savedLocText);
        }

        if (!getCookie('visitor_location_saved_v2')) {
            initUbigeo();
            // Show popover instead of modal
            document.querySelectorAll('.locationPopover').forEach(pop => pop.style.display = 'block');
            document.querySelectorAll('.popoverLocationText').forEach(el => el.innerText = savedLocText || 'Lima, Lima');
        } else {

        }

    // Event delegation para botones de ubicacion que pueden ser clonados por el sticky header
    document.body.addEventListener('click', function(e) {
        if (e.target.closest('.btnOpenUbigeo')) {
            e.preventDefault();
            initUbigeo();
            myModal.show();
        }
        if (e.target.closest('.btnConfirmLocation')) {
            e.preventDefault();
            setCookie('visitor_location_saved_v2', '1', 30);
            document.querySelectorAll('.locationPopover').forEach(pop => pop.style.display = 'none');
            if (!localStorage.getItem('visitor_location_text')) {
                localStorage.setItem('visitor_location_text', 'Lima, Lima');
            }
        }
        if (e.target.closest('.btnChangeLocation')) {
            e.preventDefault();
            document.querySelectorAll('.locationPopover').forEach(pop => pop.style.display = 'none');
            initUbigeo();
            myModal.show();
        }
        if (e.target.closest('.btnClosePopover')) {
            e.preventDefault();
